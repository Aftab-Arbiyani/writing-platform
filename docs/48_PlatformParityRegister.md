...
| **W5-12**           | harness      | ~~three visual baselines do not reproduce outside CI (~21 px page offset)~~ **The three W5-12 baselines are FIXED and the 2026-09-01 re-mint confirms it: of 124 regenerated files, **121 were byte-identical** and the only three that changed were `frontend-reader` (chromium/dark/firefox) — a legitimate C-15 change, reviewed image-by-image before committing (the new "Suggest an edit" control above the prose, and the ~62 px reflow it causes). `frontend-suggestions` did NOT change, which falsifies this pass's own prediction that removing its header button would move it — `CapabilityGate` fails closed, so that button never rendered in the shot | `frontend-comments`, `frontend-suggestions`, `frontend-collaborators`                                                                                                                                                                                         | **CLOSED for the drift.** What remains is NOT this row: two **webkit** @visual arrangements fail, so `frontend-reader-frontend-webkit-linux.png` was not re-minted and is now stale against its three siblings. Filed as **WK-visual** below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **AA-render**       | verification | ~~the W8-5 / T-4 pins have never been rendered~~ **DISCHARGED 2026-08-20** for the a11y half: 94/94 axe checks green across frontend+admin × light+dark, and a NEW rendered guard measures the hover and press fills directly. **Still owed: the visual half** — baselines are verified only in `mcr.microsoft.com/playwright:v1.61.1-noble` (10 §8.3), and a bare-host run is meaningless by design                                                                                                                                                                                                                                                                  | `e2e/tests/frontend/a11y.spec.ts` — "a hovered and a PRESSED primary button both clear AA". Owed: `web-e2e-visual` in the pinned image, which is the **CI** row below                                                                                         | ~~**0.2 d** (it is the CI row)~~ **RUN 2026-08-25 and the 0.2 d was wrong: 73 failed / 2 flaky / 49 passed.** 44 of those are baselines that NEVER EXISTED (76 committed vs 124 visual tests — every visual test written since the 2026-07-30 mint), 23 are genuine drift. The job cannot pass without a **reviewed baseline regeneration** ([10 §2.2]), which is an OWNER DECISION, not an estimate. See [§3.25a](#325a-the-engine-sweep-and-the-visual-job-that-cannot-pass-2026-08-25)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **AI-panel-visual** | **medium**   | the four `@visual` AI-panel baselines cannot be minted: the test runs **flags-DOWN** on purpose (its baseline pins the "AI is turned off" state), but **B5** made the editor's AI trigger conditional on `aiAvailability !== 'off' && !== 'self-off'` — so in that exact state the trigger is not rendered. Observed as `locator.click: Timeout 15000ms` ×4 in the 2026-08-25 pinned-image run, the button resolving but never becoming stable/enabled. **Same false premise §3.23a found in `assistant.spec.ts`**, which was fixed by RAISING the flags — a fix this test cannot take, because flags-down IS its subject                                             | `e2e/tests/frontend/visual.spec.ts:85` (`withAiFlags('visual: AI panel (flags down)')` → `AssistantPanel.open()`); the condition is `frontend/src/features/writing/pages/editor-page.tsx:228` (B5, `2797b69`). Opened 2026-08-25 by the engine sweep (§3.25a) | **0.5 d+** (needs a product answer first: what SHOULD a flags-down editor offer as an entry point, if anything — B5 deliberately removed the stranded one. The baseline follows that answer, not the reverse)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **WK-visual**       | harness      | **Two `@visual` arrangements fail on WEBKIT only** (run #32, 2026-09-01, regenerate mode), both in specs this pass reworked. `the suggestions page…`: `locator.click` timed out on "Suggest an edit" with the log reading _"locator resolved to `<button …`"_ — found, never stable. `the reader…`: `toHaveText` on the `h1` reported **element(s) not found**. Chromium, dark and firefox all passed the same specs and re-minted cleanly, so this is not the arrangement being wrong in general. **Consequence:** `frontend-reader-frontend-webkit-linux.png` was not regenerated and is now stale against its three siblings                                       | `e2e/tests/frontend/visual.spec.ts` (the reader-driven `proposeEdit` arrangement); `playwright-report-visual` from run 33497746515                                                                                                                            | **unsized — needs the pinned image to reproduce.** Plausible and UNPROVEN: the affordance re-renders as `CapabilityGate` resolves (fails closed, then opens), so a click can land during the flicker — webkit being slowest would lose that race first, which is the §3.25e shape. Do not act on that guess without counting it. **Non-blocking today**: `@visual` is `continue-on-error` AND `workflow_dispatch`-only, and webkit is off the push matrix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **W6** + **A4** | the AF3 analysis lifecycle and its admin half — both held. Note the interaction D4 created: the lifecycle's `analyze` trigger is now a **gated** capability (`story_intelligence`), so whichever row ships it ships the first client that can spend it. **Absence re-verified 2026-08-31:** grepping `frontend/src`, `admin/src` and `qalam-mobile/lib` for `stories/*/analyze` and `/analyses` returns **nothing** on any of the three; the single `story-intelligence` hit is `writing-assistant-panel.tsx`, which is D4's premium GATE, not a lifecycle client | 2026-08-31                |
