...
| **WK**              | verification | ~~the frontend webkit shards have not been re-run~~ **RUN 2026-08-20, and the row's frame was wrong.** Webkit cannot launch on a dev host at all (missing `libgstcodecparsers`/`libavif` — `README` says so). In the pinned image: **140 passed / 7 failed / 3 flaky**, and an identical chromium control failed **6 of the same 7**. No engine-specific deterministic failure in the frontend suite either — same verdict §6.18 reached for admin                                                                                                                                                                               | the shared failures are `assistant.spec.ts` ×3 + `a11y.spec.ts:227`, now **AI-flag** in [3.22c](#322c-harness--the-suites-own-honesty); collaboration ×3 was B6's seat cap and is FIXED                                                                       | **done** for the original question. ⚠️ **A 2026-08-31 closure of this row was WRONG and is retracted the same day.** Run #24 passed both webkit shards (10.2 / 8.9 min) and that was recorded as "the webkit thread is closed". **Run #25, on the IDENTICAL commit `08e8060`, failed both webkit shards** — so the pass was one observation, not stability, and closing on it repeated the exact error this register keeps catching in the other direction (§3.4: a failure is not a flake until counted — and its converse, a single pass is not a fix). Webkit is **1 green / 1 red on the same SHA**, uncounted. See [§3.25d](#325d-webkit-is-not-stable-and-325c-closed-it-on-one-run-2026-08-31). ✅ **COUNTED AND ANSWERED 2026-08-31 — [§3.25e](#325e-webkit-was-never-engine-specific-behaviour--it-was-engine-specific-timing-2026-08-31), `585d7fd`.** Never engine-specific behaviour: an axe **mid-fade race** in the a11y fixture, with an exact arithmetic explanation (`#7e786e` = `--q-text-muted` `#726c61` at α≈0.916; settled = 5.207:1, AA-compliant). Counted in the pinned image on webkit: **155 contrast hits → 0**, 9 flaky/94 passed → **103 passed/0 flaky**. Webkit then went green in #26, #28 and #29. **This row is closed as a question about the ENGINE.** What replaces it is not a defect: **[§3.25g](#325h-runs-28-to-30-the-matrix-narrowed-to-four-jobs-and-the-count-reset-2026-08-31) — webkit is off the PUSH matrix by owner call on time cost**, restorable in one word |
