...
| 7   | **Onboarding**            | ✅ **CLOSED 2026-08-31** — `/onboarding`, a port of mobile's three-slide carousel with the same copy, shown once per browser and handing off to sign-in. The intercept is in `HomeRoute` (root only), NOT a tree-wide guard                                                                                                                                                                                                                                                   | **Owner decision 2026-08-31: build it.** Was the last §2 row with no owner. See [§3.26](#326-onboarding-was-ported-and-a-tree-wide-guard-would-have-hijacked-every-shared-link-2026-08-31)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **unassigned — see §5**                                                                                                                                                                                                                                                                          |
...
### M-5 · **medium** · ~~**OPEN — ledger §3.22c**~~ **CLOSED 2026-09-01 — NOT REPRODUCIBLE** · mobile's suite fails ~2 runs in 10, with no assertion output (opened 2026-08-03, during the parity batch)

> **Closed by owner decision, on the count rather than on a fix — and the count is the justification.**
> §3.4's rule is that a failure is not a flake until it has been counted; the converse applies here.
> **23 combined runs produced 0 failures** (2026-08-21: the file alone 15/15, full-suite 7 runs at
> default concurrency + 1 at `--concurrency=48`), where the recorded ~2-in-10 rate predicts 4–5. The
> hardening fix (`bc74d29`, flushing a dangling `unawaited(record(...))` before disposal) is a real
> defect fixed, and remains explicitly **not** a proven fix for this symptom.
>
> **What closing it actually means, stated so nobody re-opens it from the wrong end:** the original
> ~2-in-10 environment was never recorded (rule 1 territory), so this can be neither reproduced nor
> refuted. If it recurs, the first step is unchanged — `flutter test --reporter expanded`, count
> before diagnosing — and it should be filed as a NEW row with its environment named, not resurrected.
...
### AI-2 · **low** · ~~**OPEN — ledger §3.22b**~~ **CLOSED 2026-08-31** · a stack running an inert AI provider reports its AI as `inert`, which understates it

> **Closed with the three-state vocabulary the row asked for, on BOTH indicators as it required.**
> `live` = a real credential; `test` = the stub/manual adapter is the ACTIVE path
> (`AiHealthIndicator` needs `defaultProvider === 'stub'` **and** `stub.enabled`, since either
> alone is genuinely inert; `PaymentHealthIndicator` needs `manual.enabled`); `inert` = neither. A
> real processor outranks manual, because where a customer's money goes is what an operator needs
> to see. Anchors: `backend/src/health/indicators/{ai,payment}.health-indicator.ts`, five new cases
> in `infra-indicators.spec.ts`.
>
> **One thing the row did not contain:** the existing spec stubs omitted `stub`/`manual` entirely
> (they are deliberately partial `as unknown as` casts), so reading those gates crashed three tests
> that had been passing. The stubs were completed rather than the code made defensive — the real
> config type guarantees both keys, and optional-chaining them would have hidden the next such gap.
...
### AI-3 · **low** · ~~**OPEN — ledger §3.22b**~~ **CLOSED 2026-08-31** · `IMPLEMENTED_AI_PROVIDERS` and `IMPLEMENTED_PAYMENT_PROVIDERS` are dead exports

> **Closed, and the two halves got OPPOSITE answers — which is the finding.**
>
> - **`IMPLEMENTED_AI_PROVIDERS` is now load-bearing.** Admin's AI config Select was
>   `Object.values(AiProvider)` — **nine** options for **three** working adapters, so an operator
>   could save `ollama` and have every later AI call fail. It is built from the list now. `stub` stays
>   out although it HAS an adapter: it is a test-stack path, and offering it in production is how
>   every writer's suggestion becomes the same canned paragraph.
> - **`IMPLEMENTED_PAYMENT_PROVIDERS` must NOT gate anything, and nearly did.** The obvious
>   narrowing is the three DTOs carrying `@IsIn(Object.values(PaymentProvider))`. That would have
>   **broken the E2E stack and preview**: `Manual` is a working, shipped adapter
>   (`payments/adapters/manual.adapter.ts`, with its own spec) deliberately absent from the list
>   because it is not a _processor_, and those deployments bill through it. It stays a reference list
>   with that reason recorded on it. There is no admin payment-provider picker to feed — the provider
>   is chosen per-purchase by the client flow, never configured org-wide.
>
> **A safety point the row did not raise:** narrowing a picker must not silently drop a value an
> operator already saved. A stored non-implemented provider stays selectable, labelled
...
### W5-12 · harness (pre-existing) · ~~**OPEN — ledger §3.22c**~~ **CLOSED 2026-09-01 — FIXED AND VERIFIED** · three visual baselines do not reproduce outside CI

> **The only row on this list closed by a fix that was then proved.** Root cause was scroll POSITION,
> not row count and not stitching (§3.25f corrected §3.25a, then measured it: expected content at row
> 103, actual at 128 — 25 px, exactly the page's maximum scroll). `atScrollTop` pins the capture at 0.
>
> **Proof, from run #32's regeneration:** of **124** baselines re-minted, **121 were byte-identical**.
> The three that changed were `frontend-reader` (chromium/dark/firefox) and are a legitimate C-15
> change — reviewed image-by-image, then committed (`d38df85`). A drift that reproduced on every mint
> now reproduces on none.
...
> reads as "something went wrong" is not closed. **Opened by this fix: [B8-2](#b8-2--low--open--ledger-322a-closed-2026-08-31--granting-an-override-to-a-nonexistent-id-inserts-a-row-that-can-never-apply-opened-2026-08-20).**
> Backend + admin + e2e; 9 new backend assertions, verified by removing the guard and watching all
> nine fail.

### B8-2 · **low** · ~~**OPEN — ledger §3.22a**~~ **CLOSED 2026-08-31** · granting an override to a nonexistent id inserts a row that can never apply (opened 2026-08-20)

> **Closed with validation at the write path, and the FK question answered NO on evidence.**
>
> B8-1 left the writes alone and recorded that the three needed separate answers rather than one
> rule applied three times. They did: **`POST overrides`** and **`POST credits/adjust`** take a
> `userId` in the BODY, so nothing upstream has proven it — both call the existing
> `assertUserExists` now. **`POST payments/:id/refund` deliberately does not**: it is keyed by a
> PAYMENT id, and a payment that exists already carries a real `userId`, so a second lookup would
> answer a question the path has answered. That absence is pinned by a test so a later pass does
> not "fix" it into a redundant query.
>
> **No migration, and that is the substance of the FK answer.** All **12** monetization entities
> declare **zero** relations and **10** carry a bare `uuid userId` — "no FK to users" is the
> module's convention, not an oversight in this one table. An FK on `entitlement_overrides` alone
> would need a lock-taking `ALTER` on a populated table, would leave it inconsistent with eleven
> siblings, and would still not protect the other nine. One query on an admin-only route is the
> same answer for every table that ever needs it.
>
> Five new cases, including the one that would otherwise rot: a NEGATIVE credit adjustment takes
> the `debit` branch, so the assert has to sit ABOVE the sign fork — pinned, because moving it
> inside either branch still passes the positive-amount test.
...
suggests — the coordinates were right and never moved. See the sweep in [§6.19](#619-318b--the-lost-action-menu-click-closed-2026-08-18)
...
### A3-4 · **low** · ~~**OPEN — ledger §3.22b (recorded, not fixed)**~~ **CLOSED 2026-08-31** · `AsyncSection` now exists five times, and the refactor keeps being "not this row's"

> **Lifted to `admin/src/components/async-section.tsx`; all five copies deleted, 26 importers
> repointed.** The fifth copy's own docblock had called it: _"five is the number at which the
> refactor stops being hypothetical."_
>
> **It was never an exception to the deletability rule, which is why it could just be done.**
> `admin/src/features/README.md` states both halves: a feature may not import another feature, AND
> **"Cross-cutting pieces move up to `src/components/`."** Every copy cited the first half and none
> cited the second.
>
> **Checked rather than assumed before consolidating:** the five files had five different MD5s and
> three different line counts, which looks like divergence. Stripping the docblocks made all five
> **byte-identical** — only the prose differed, each describing whichever slice wrote it. Had the
> bodies actually differed, this would have been a behaviour merge, not a delete.
>
> Verified: admin typecheck, eslint, build and **70 files / 400 tests** all green.
...
> **STATE OF THE LEDGER, 2026-09-02: 3.22a, 3.22b and 3.22c are all EMPTY. One row remains anywhere
> in it — `B2` in 3.22e — and it is an owner hold, not work.**
>
> `CI` was the last open row and it closed today, which is worth one paragraph because of _how_ it
> closed. It sat at "0.5 d" for weeks, and it was never a half-day of engineering: what it needed
> was a **definition**. Three consecutive green runs of _what_, exactly, after the matrix had been
> narrowed from seven jobs to four? Answering that (owner decision, written at its source in
> [e2e/07 §6.2](./e2e/07_CI.md)) took one pass, and the count then completed in two pushes — #32,
> #33, #34, all four `web-e2e` jobs green in each.
>
> **Answering it also exposed that the criterion had been UNREACHABLE as written**, which no amount
> of running the suite would have revealed: §6.1's promotion step required `web-e2e-visual` green
> three consecutive times, and that job has not run on a push since `11f5efb`. Runs #33 and #34 both
> reported it **`skipped`** — the proof arriving by measurement after the reasoning. That is the
> second time a gap in a **design doc** rather than in code has cost this suite a run (the first:
> the missing `timeout > expect.timeout` invariant that reddened #30), and it is the argument for
> writing a criterion where it lives instead of only where it is tracked.
>
> **What is left is deliberately not in this ledger.** The four `web-e2e` jobs still need marking
> **required in branch protection** — a repo-settings change with no `file:line` anchor, so rule 1
> cannot police it and it would be a fiction as a row. It is an owner action, recorded in §6.2.
...
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | ----------- |
| ~~**D5-clients** · **high**~~ **CLOSED 2026-09-07** | ~~Both clients call routes B2 deleted.~~ **Web half closed by `b349798`** — the Ask tab, conversation pages, prompt library and `/settings/ai/usage` deleted, and `useAiAvailability` no longer reads the removed `GET /ai/usage/me`, which had been 404ing on every editor load. **Mobile half closed by `8e6e302` (M1)** — Ask My Book, both conversation screens, the prompt library and the AI-usage screen deleted with their routes; `AiRepository` is down to five methods, none of which touches a removed endpoint. Nothing 404'd in production — nothing had deployed | 2026-09-03, by `7f3b459`; web half 2026-09-03, mobile half 2026-09-07 | —           |
...
**EMPTY as of 2026-08-31** — **AI-2**, **AI-3** and **A3-4** all closed that day.
...
~~**Fix size is unknown until measured**, so each estimate is a measurement loop plus an unbounded
fix. Naming that is the point: §3.4's rule is that a failure is not a flake until it has been
counted.~~

**EMPTY as of 2026-09-02 — `CI` was the last row and its criterion is satisfied** (`#32 ✓ · #33 ✓ ·
#34 ✓`, three consecutive runs with all four `web-e2e` jobs green). The `pull_request:` trigger is
in, so the browser suite runs on every PR. **The one thing still outstanding is not a ledger row
and never could be:** marking those four jobs **required in branch protection** is a
repo-settings change, so it has no `file:line` anchor for rule 1 to police. It is recorded as an
owner action in [e2e/07 §6.2](./e2e/07_CI.md) instead.

The estimate note struck above is kept for its lesson, which outlived its table: **fix size is
unknown until measured**, and §3.4's rule is that a failure is not a flake until it has been
counted. `CI` is the row that proved it from the other direction — it was sized at "0.5 d" for
...
> the tidiness.** The instruction was to close everything but **B2** and the **CI** criterion. Two of
> those rows were genuinely fixed and verified (**W5-12**, **AA-render**) and their §3 diagnoses are
> struck as such. The rest are **accepted, not repaired** — so they are recorded here, in the table
> whose entire purpose is "a future row does not size them as work", rather than written up as fixes.
>
> Calling an unrepaired thing "fixed" is the one move that would make this register worse than having
> no register: every rule in §3.22 exists because a status claim outlived its evidence. So the evidence
> is below, and it says "not fixed" out loud.

- **`AA-render`'s visual half — DISCHARGED 2026-09-01.** The row's remaining ask was a reviewed
  baseline regeneration in the pinned image, and run #32 (`workflow_dispatch`,
  `update_visual_baselines`) did exactly that: 124 regenerated, 121 byte-identical, 3 reviewed and
  committed. Its a11y half was already discharged 2026-08-20 (94/94 axe checks + a rendered
  hover/press guard). Nothing is owed.

- **`AI-panel-visual` — ACCEPTED, and the product question is answered by NOT answering it.** The four
  flags-down AI-panel baselines are deliberately **never minted**, and the test stays `test.fixme`
  (`e2e/tests/frontend/visual.spec.ts`). The row existed because the spec runs flags-DOWN while **B5**
  (`2797b69`) removed the editor's AI trigger in exactly that state, so `AssistantPanel.open()` clicks
  a button that is not rendered. **B5's decision stands: a flags-down editor offers no AI entry point,
  and that is correct** — a stranded trigger for a switched-off subsystem is worse than none. There is
  therefore nothing for a baseline to depict. **If** a flags-down entry point is ever introduced, this
  becomes a new row with a new subject; it is not this one reopening.

- **`WK-visual` — NOT FIXED. Accepted as known, non-blocking harness debt.** Two `@visual`
  arrangements fail on **webkit only** (run #32, 2026-09-01, regenerate mode): the suggestions-page
  shot's click on "Suggest an edit" logs _"locator resolved to `<button …`"_ and then times out at
  15 s, and the reader shot's `h1` is reported **element(s) not found**. Chromium, dark and firefox
  passed the same specs and re-minted cleanly.
  **The live consequence, so nobody discovers it as a surprise:**
  `frontend-reader-frontend-webkit-linux.png` was **not** regenerated and is now stale against its
  three siblings — it predates the "Suggest an edit" control. A future dispatch will fail on it until
  the webkit arrangement is sorted.
  Why accepting it is defensible rather than lazy: `@visual` is `continue-on-error` **and**
  `workflow_dispatch`-only, and webkit is off the push matrix (`11f5efb`) — so it cannot fail a run,
  cannot block the PR gate, and no user-facing behaviour depends on it. A plausible cause is recorded
  and **explicitly unproven**: the affordance re-renders as `CapabilityGate` resolves (fails closed,
  then opens), so a click can land during the flicker, and webkit being slowest loses that race first
  — the §3.25e shape. **Count it before acting on it**, in the pinned image.

- **The AF3 analysis lifecycle (`W6` + `A4`) — OUT OF SCOPE, by owner decision 2026-09-01.** Seven
  backend routes with no client on any platform (`POST /stories/:id/analyze`, `/graph`,
  `/graph/characters`, `/timeline`, `/analyses`, `/analyses/:id`, `DELETE /graph`) — re-verified absent
  that day: grepping `frontend/src`, `admin/src` and `qalam-mobile/lib` returns nothing, the single
  `story-intelligence` hit being D4's premium gate rather than a lifecycle client.
  It is **not** engineering-blocked; it is product-undefined, and the questions are real ones: who
  triggers an analysis and when, what it costs against the AI budget, whether extracted entities are
  the writer's to confirm or correct, and what resetting a graph means for anything built on it.
  `docs/35` scoped clients out and nothing has scoped them back in. **D4 raised the stakes**: `analyze`
...
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| **B2** | held 2026-07-29. Its "shares D4's enforcement path and writes the first real `isEntitled` caller" is **spent** — D4's build wrote that caller (`StoryIntelligenceService.assertGraphReadEntitled`, `story-intelligence.service.ts`), so B2 now inherits an established pattern instead of setting one, and holds only `premium_content` itself | 2026-08-31                |

> **Follow requests is no longer in this table either — it was never a decision, it was already
> BUILT, and it had been for seven weeks.** Removed 2026-08-31. Web ships the whole surface:
> `app/routes/follow-requests.tsx` → `/me/follow-requests`,
> `features/profile/pages/follow-requests-page.tsx` (with its own spec),
> `components/follow-request-row.tsx`, `hooks/use-follow-requests.ts`, and real calls to
> `PATCH /follow-requests/:id/{accept,reject}` — cursor-infinite, with empty/error/loading states.
> It landed in **`a890d85`, 2026-07-09**, which is _before_ this register's §2 row 4 was written to
> say web lacked it. Mobile has `follow_requests_screen.dart`. Both platforms ship it.
>
> **This is the `register-findings-go-stale` lesson recurring in the one table that cannot catch it.** The tables above are
> policed by rule 1's anchor requirement — an entry needs a `file:line` and a verification date — and
> this table has no equivalent, which is exactly why the 2026-08-24 pass added the "last confirmed"
> column after D4. That column was not enough on its own: four rows were re-confirmed on 2026-08-24
> and this one was re-confirmed _without anybody grepping `frontend/src` for it_. Re-dating is not
> re-verifying, and a held row needs the same evidence standard as a defect: **name the absence, with
> the path you checked.** The Onboarding row does this correctly ("Confirmed absent: no
> `frontend/src/features/onboarding`") and is the model for the rest.
>
...
### 3.25a The engine sweep, and the `@visual` job that cannot pass (2026-08-25)

The rest of what run 4 could not speak for, measured. **Firefox is fine. `@visual` is not, and has
not been for weeks.**

| Leg                                                                            | Result                              |
| ------------------------------------------------------------------------------ | ----------------------------------- |
| `frontend-firefox` + `admin-firefox` functional, 2 workers                     | **232 passed**, exit 0              |
| `@visual`, all 8 projects, pinned image `v1.61.1-noble`, CI's exact invocation | **73 failed · 2 flaky · 49 passed** |

**The visual job's failures are not one problem, and the first reading of them was wrong.** They look
at a glance like baseline drift — 15+ UI-changing commits have landed since the baselines were
committed (`c993e18`, 2026-07-30), including `a47eb50`, which repainted accent hover/press fills and
derived the chart palette app-wide. Drift is real, but it is the **minority**:

| Cause                        | Count  | What it is                                                                                                                          |
| ---------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| **A snapshot doesn't exist** | **44** | Visual tests added AFTER the mint. Never had a baseline, so `CI=1` fails them outright                                              |
| Pixel diff                   | 23     | Genuine drift — mostly `a47eb50`'s palette, plus D3/D4's gates changing what AI pages render                                        |
| `locator.click` timeout      | 4      | The "AI assistant" button resolves, then never becomes stable/enabled. §3.23's family, a new manifestation — **not diagnosed here** |
| `409 COLLECTION_NAME_TAKEN`  | 4      | An arrangement collision. **Fixed** (below)                                                                                         |

**The dominant cause is coverage, not drift, and the arithmetic says so plainly: 76 committed
baselines against 124 visual tests.** Per project the set is 16 frontend / 3 admin — while
`admin/visual.spec.ts` alone now drives the users console, plans console, billing actions, retrieval
config and trust standing, all added by A2/A3 in mid-August. **Every visual test written since
2026-07-30 has been guaranteed-red**, and nothing said so, because this job has not run since it was
minted. That is §3.24's lesson a third time: the gates most likely to be red are the ones no row's
definition of done names.

**One real spec defect, found and fixed.** `visual.spec.ts` created a collection titled
`'E2E Collection — visual baseline'` — hard-coded, while all eight projects run the spec as the same
writer, so the second arrival failed in ARRANGE with a 409. It broke [04 §5]'s own rule ("never create
a record with a fixed name") three lines from an already-injected `data` fixture. Randomising would
have broken the screenshot instead, since the name is RENDERED in the image the baseline pins — so
the title is now keyed to `test.info().project.name`: stable within a project, unique across them,
free because baselines are already namespaced per project.

**What this does to AA-render and the CI row.** **AA-render's visual half is NOT dischargeable** — it
was sized 0.2 d ("it is the CI row") on the assumption that running the job was the whole cost. The
job cannot pass until 44 baselines are minted and 23 are refreshed, and **that is a reviewed
operation, never a blind accept** ([10 §2.2]); `web-e2e.yml` carries a `workflow_dispatch` input
(`update_visual_baselines`) precisely so a human approves the PNGs. It is therefore **an owner
decision, not an engineering estimate**, and it is not taken here. **W5-12**'s three
non-reproducing baselines are subsumed by that refresh and should be re-judged after it, not before.

The `CI` row's functional half is in good shape — chromium and firefox both 232/232 at CI's
concurrency. Its visual half is blocked on the baseline decision above.

**Decision taken 2026-08-25: regenerate, and let CI mint them.** Not locally, and the reason is worth
recording because it nearly went the other way. This machine's E2E database holds **5,954 pieces, 100
collections and 3,344 users** for the seeded writer, against a handful on CI's freshly-seeded one —
and several visual specs shoot `fullPage`. Baselines minted here would encode weeks of accumulated
test data and fail in CI on arrival, wasting the review they exist to enable. **That is also the best
available explanation for W5-12** ("three visual baselines do not reproduce outside CI, ~21 px page
offset"): data volume changes page height. So regeneration goes through
`workflow_dispatch(update_visual_baselines: true)`, which seeds fresh, mints in the pinned image, and
uploads the PNGs as an artifact for review — the path that input was added for.

**Regeneration will not make the job green on its own**, and the gap is now a row rather than a
surprise: **AI-panel-visual** (4 tests) fails before it reaches a screenshot, so those four baselines
cannot be minted by this or any run until the product question behind it is answered.
...
### 3.25c Run #24 — the functional suite is GREEN in CI, on all three engines (2026-08-31)

The next run did reach a test, and then passed. **First time this workflow has ever been green on the
functional side**, and the first time **webkit has been verified anywhere outside a one-off
experiment.**

| Job                     | Result                                      | Time     |
| ----------------------- | ------------------------------------------- | -------- |
| `web-e2e (chromium, 1)` | **success**                                 | 6.2 min  |
| `web-e2e (chromium, 2)` | **success**                                 | 7.3 min  |
| `web-e2e (firefox, 1)`  | **success**                                 | 7.2 min  |
| `web-e2e (firefox, 2)`  | **success**                                 | 7.0 min  |
| `web-e2e (webkit, 1)`   | **success**                                 | 10.2 min |
| `web-e2e (webkit, 2)`   | **success**                                 | 8.9 min  |
| `web-e2e-visual`        | failure — `Run @visual in the pinned image` | 17.6 min |

**Webkit going green settles the question §6.18 and the 2026-08-20 run left half-open.** That run
found 7 webkit failures in the pinned image with a chromium control failing 6 of the same 7 — which
argued _no engine-specific defect_ but could not prove it, since the shared failures were still real.
Those were the AI-flag family (§3.23) and B6's seat cap (§3.22c), both since fixed, and webkit now
passes both shards clean. **The "webkit is deferred / CI-only / possibly broken" thread, open since
2026-08-03, is closed.** What remains true is only that webkit cannot LAUNCH on a dev host (missing
`libgstcodecparsers`/`libavif`) — an environment limitation, not a defect.

**Timings, for whoever sizes the next run.** The functional jobs are 6–10 min; webkit is consistently
the slowest by ~2–3 min. The visual job is **17.6 min**, of which ~2.8 min is setup and the rest is
the test step — and a large part of THAT is `retries: 2` re-running ~73 tests that cannot pass, so a
regeneration run should be _faster_ than a verification run, not slower. Expect the visual job well
under 10 min once its baselines are right.

**Where the CI row now stands.** Functional: green, three engines, two shards, nothing owed. Visual:
still red, still on the baseline regeneration decided in §3.25a — and note run #24 was a **`push`**,
so it VERIFIED baselines rather than regenerating them; the regeneration needs
`workflow_dispatch(update_visual_baselines: true)`. Also worth recording for whoever runs it:
`concurrency: web-e2e-${{ github.ref }}` with `cancel-in-progress: true` means a dispatch fired while
a push run is live **cancels the push run** — that is what run #22 is, and it is easy to misread as a
flake.

---

### 3.25d Webkit is not stable, and §3.25c closed it on one run (2026-08-31)

**Run #25 — `workflow_dispatch`, `update_visual_baselines: true`, the same commit `08e8060` as run
#24.**

| Job                     | Run #24                    | Run #25                           |
| ----------------------- | -------------------------- | --------------------------------- |
| `web-e2e (chromium, 1)` | success                    | success                           |
| `web-e2e (chromium, 2)` | success                    | success                           |
| `web-e2e (firefox, 1)`  | success                    | success                           |
| `web-e2e (firefox, 2)`  | success                    | success                           |
| `web-e2e (webkit, 1)`   | **success**                | **FAILURE**                       |
| `web-e2e (webkit, 2)`   | **success**                | **FAILURE**                       |
| `web-e2e-visual`        | failure (17.6 min, verify) | failure (**9.0 min**, regenerate) |

**§3.25c's webkit closure was wrong and is retracted.** It read one green run as the end of a thread
open since 2026-08-03. The very next run, on a byte-identical commit, failed both shards. Webkit
therefore stands at **1 green / 1 red on the same SHA, uncounted** — which is not a verdict in either
direction.

**This is §3.4's rule used in only one direction, which is how it fails.** The register has been
strict for weeks that _a failure is not a flake until it has been counted_ (T-7 carried an uncounted
"flaky" label for three weeks; RS-flake was closed only after counting). The converse is the same
rule and was not applied: **a single pass is not stability.** Two engines with four consecutive greens
(chromium, firefox) are a reasonable claim; one engine with one green was not, and the fact that it
closed a long-open thread is exactly what should have made it suspicious rather than satisfying.

**What is NOT yet known**, and must not be guessed at: whether run #25's two webkit failures are the
same tests as each other, the same as the seven from 2026-08-20, or new. The step-level API says only
that step 12 (`Run E2E (webkit shard N)`) failed; **artifact download requires authentication**, so
`playwright-report-webkit-shard1/2` have not been read. Until they are, "webkit flake" is a
suspicion with a count of one, not a finding.

**The one thing run #25 did settle:** regeneration is faster than verification, as predicted —
**9.0 min against 17.6 min**, because `--update-snapshots` writes instead of comparing and stops ~73
tests being retried twice each. And it produced the artifact: `updated-visual-baselines`, **10.89 MB**.

**Consequence for the CI row.** Its exit criterion is _three CONSECUTIVE green runs_. On this evidence
chromium and firefox would pass that; webkit would not, and one red resets the count. So the row is
**not** simply waiting on the baseline commit — it is waiting on webkit being counted, which is a new
and unsized piece of work.
...
### 3.25h Runs 28 to 30, the matrix narrowed to four jobs, and the count reset (2026-08-31)

**This section exists because §3.25g stops at `06bcc29` and three commits happened after it.** The CI
and WK rows were still quoting "#24 ✓, #25 ✗, #26 ✓" and "webkit 1 green / 1 red, uncounted" — both
written before the runs that answer them. Rule 3 caught in the doc direction for once: the code moved
and the ledger did not.

| Run | SHA       | Event                                | Result                                                                                                         |
| --- | --------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| #27 | `06bcc29` | push                                 | **cancelled** at 1m16s — superseded by the #28 dispatch, the `concurrency` behaviour §3.25c already documented |
| #28 | `06bcc29` | dispatch (`update_visual_baselines`) | **success** — 6/6 functional green (webkit 6.8 / 9.3), visual red but advisory                                 |
| #29 | `31bcbf2` | push                                 | **success**                                                                                                    |
| #30 | `7090050` | push                                 | **FAILURE** — webkit                                                                                           |
...
and #29 — see §3.25e for why it was never an engine problem); `@visual` becomes `workflow_dispatch`-only
(it cost ~7 min per push while `continue-on-error`, i.e. it could not fail the run and nobody was
reading it — pure latency). Neither is deleted, both carry restore instructions in the workflow, and
restoring webkit is one word in the matrix. **Coverage knowingly given up:** webkit regressions on push,
and visual regressions between dispatches.

**⚠️ The CI row's exit criterion now needs restating, and that is an OWNER DECISION recorded here
unanswered.** "Three consecutive green runs" was written when a run meant 7 jobs across 3 engines. A
green 4-job run certifies materially less. Two readings, and they are not equivalent:

1. **The count restarts against the 4-job shape** — the gate then means "chromium + firefox, both
   shards, three runs running", with webkit and visual verified by dispatch on a cadence someone owns.
2. **The criterion keeps its original meaning** — in which case the flip waits on a 7-job green streak,
   and `11f5efb` has to be reverted or the matrix made event-dependent before the count can resume.

Reading 1 is cheaper and matches what the functional suite has actually been catching all month;
reading 2 is what the row literally says today. **Not decided here** — a criterion is not something a
reconciliation pass gets to quietly loosen, which is the whole reason D4 taught this register to keep
decisions and fixes in separate tables.

**Two things left unverified, both because of the narrowing itself:** `7090050`'s
`collaborators`-off-`fullPage`-plus-`atScrollTop` fix (the third W5-12 baseline) has **never run** —
`11f5efb` had produced no run at all as of this writing — and the two stale baselines §3.25g names
(`frontend-suggestions` at scroll 25, `admin-users-*-dark` half-loaded) still need their re-mint
dispatch. `@visual` being dispatch-only means neither will be answered by simply pushing.

**Also fixed in this pass, and it is worth naming because no gate could have caught it:** `06bcc29`
**deleted the `## 4. Divergences that are NOT gaps (platform-inherent)` heading** while inserting
§3.25g, so a whole top-level section's content sat orphaned under a §3.25 subsection and every
cross-reference to §4 in this document pointed at a heading that no longer existed. Restored. A
markdown structural break is invisible to `git diff --stat`, invisible to review at the hunk level, and
this register cross-references §4 from §1, §2, §3 and §6.

## 3.26 Onboarding was ported, and a tree-wide guard would have hijacked every shared link (2026-08-31)

**Built at the owner's decision.** It was the last §2 row with no owner at all, and it was held on "a
product shape for web" — which turned out not to be needed, because the shape already existed on the
other platform. Under §1 web ships the same product, so this is a **port of mobile's
`onboarding_screen.dart`**, not a web original: the same three slides in the same order
(welcome · platform · privacy), the **same copy verbatim** from `app_en.arb`, the same dot
indicators, the same "Next" that becomes "Get started", and the same hand-off — Skip and Get started
are treated identically, both persisting completion and going to sign-in.

Nothing was designed that could be copied. Where mobile's structure had a reason, that reason
transferred with it: mobile keeps the durable flag in `core/session/` rather than in the onboarding
feature because "the router guard and the launch-phase provider read it; a feature-owned provider
would force a cross-feature/app import". Web has the identical constraint (docs/26 §4), so the flag
is `stores/onboarding.store.ts` (zustand + `persist`, after `theme.store`) and the feature owns only
the carousel.

**The one thing that did NOT transfer is the whole finding.** The first implementation followed
mobile exactly — a pathless `RequireOnboarding` guard over the chrome tree — and that is **wrong on
web**, for a reason mobile cannot encounter:

> An app always starts at its own root. **The web is entered at an arbitrary URL.** A tree-wide
> guard means a first-time visitor opening a shared story link from a message gets a three-slide
> introduction instead of the story they were sent.

Mobile's guard already contains the seed of this — it exempts the auth corridor so "a fresh-install
deep link still works" — and on web _every_ arrival is a deep link. So the intercept moved to
`HomeRoute`, the one place that already answers "where does a visitor at the root go", with a
deliberately narrow condition: **an anonymous visitor at `/` who has not seen it.** The guard was
deleted rather than left mounted somewhere harmless.

**It would also have disarmed the browser suite**, which is the fifth instance of that pattern after
B4-1, B6, D3 and D4 — and the second caught _before_ landing rather than by a red run. Every
anonymous spec would have been redirected, `reader.spec.ts`'s cold slug load included; a redirect
into an intro reads like a broken selector, exactly as B6's 402 did.

**Three things worth keeping for the next port:**

- **The flag is per-browser, and that is stated rather than hidden.** `localStorage` is the honest
  analogue of mobile's device preference, so a second browser or a cleared profile sees the intro
  again. Making it per-account needs a server field the frozen `v1` does not have, and inventing one
  for a twenty-second intro is not a trade worth making. What makes it invisible in practice is that
  **an authenticated visitor is exempt regardless of the flag** — a session is proof they have been
  here.
- **Two mobile behaviours were deliberately not ported**: a haptic on page change (no web analogue)
  and a swipeable `PageView`. Web advances by button, which is also what makes the carousel
  keyboard-operable with no extra work — the control that moves it is a real button rather than a
  gesture needing a fallback.
- **The a11y scan lives in `a11y.spec.ts`, not beside its own functional spec**, for a mechanical
  reason worth not rediscovering: `frontend-dark` matches only `(a11y|visual).spec.ts`
  (`UI_QUALITY_ONLY`), so a scan in any other file is **light-theme only**. It also needs an explicit
...
**Partly inherent (item 9 above) — DECIDED 2026-08-31, and no longer a held row.** Offline _reading_
was always inherent; the **offline write queue** (like/bookmark/follow taken offline, reconciled on
reconnect) is a real product behaviour that web lacks. It was carried in 3.22e specifically so someone
would decide it "explicitly rather than by omission". **The owner decided: accepted divergence, with
web pointing at the mobile app for it.**

- **Web's absence is total and deliberate, not an oversight.** `frontend/public/sw.js:4` says so in
  its own words — _"This file is NOT registered by default. The app ships no offline
  synchronization"_. There is no queue to finish, only one to build.
- **What it would cost, which is why it is not a small parity fix.** Mobile's is a durable Hive
  outbox behind a unified `SyncEngine` with per-verb merge semantics — and those semantics are not
  uniform: `SocialSyncHandler` carries a terminal `desired: bool` (latest-wins) while claps needed
  their **own** handler because a quantity must SUM or queued claps vanish (§4.1, M7-3). A web port
  is a registered service worker, a durable store, and that same per-verb merge matrix rebuilt.
- **The honest client behaviour instead of silence.** Web's `OfflineBanner` now says engagement is
  not saved while offline and names the mobile app as the surface that does queue it. A reader who
  taps Like on a dead connection is told what happened, which is the part that was actually missing —
  the divergence was never the defect, the silence about it was.

**Not to be built** unless a future epic revisits it with the merge matrix in scope.
...
`D5-clients` in [§3.22a](#322a-product-defects--a-user-or-an-operator-can-hit-these) is **CLOSED**
(2026-09-07): both clients are off everything B2 deleted. Mobile's half landed as M1–M4.
...
| **M1**  | Mobile: Polish sheet, Manuscript feedback, Story Map + "Map this story"; conversations/prompts/usage/Ask + the B5 screen gone    | ✅ `8e6e302` |
...
**Rows this CLOSES by building, not by striking.** The AF3 analysis-lifecycle row in
[§3.22d](#322d-accepted-not-fixed) recorded that the graph platform shipped with **no client able to
spend `analyze`** — so every Story Map view said "nothing here yet", permanently. Both clients now
have "Map this story" (F1b, M1), which is the first thing anywhere that can fill a graph. The rest of
that row stands: there is still no reset/rebuild and no confirm/correct for entities, and those
remain product-undefined rather than unbuilt.

**What mobile's half taught, recorded because it was not in the plan:**

- **A latent lifetime bug in the Polish session controller** (M1). It only ever `read` the
  autoDispose stream controller, so between starting a stream and the sheet's first rebuild the
  provider could be swept — cancelling the subscription, hanging the run in `streaming` forever, and
  mounting a _second_ instance for the view. Invisible against a real network; wide open against a
  fake answering in microtasks, which is how a widget test found it. Now `listen`ed.
- **Mobile's auth split was enforced by accident**, exactly as web's was (§3.25). Two retrieval calls
  are public and four need a session; what had been keeping the authenticated ones off public pages
  was a feature flag. Mobile is the worse case, because `AiFeatures.isEnabled` answers **false for a
  flag it cannot find** — so Phase V deleting a flag row would have taken mobile's search dark while
  web carried on. It is stated at the request now, and `search_anonymous_test.dart` arranges
  `aiFeaturesProvider` to **throw**, because a render-only check cannot tell "not shown" from
  "requested and 401'd".
- **The retrieval engine has no offline replay.** Deleting the grouped `GET /search` preview took its
  object cache with it, so a mobile reader offline on the `All` scope now sees an error where the
  per-type tabs still show their last results. Real gap, recorded rather than papered over.
