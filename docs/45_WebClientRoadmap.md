...
| **D3** | **Decision:** is the free tier meant to have AI writing, or only a token budget? ✅ **ANSWERED 2026-08-08 — NEITHER: free gets no AI writing**                                              | S         | [48 §5.2](./48_PlatformParityRegister.md). `monetization.plans` grants `free` the `ai_budget` feature with a 20k/day, 200k/month allowance but **not** `ai_writing`. If the server ever enforced `ai_writing`, that allowance becomes unspendable. **Decided by the owner: AI writing is a paid capability; free gets neither the code nor a spendable budget.** Two consequences, both required for the decision to take effect at all: (a) free's `ai_budget` allowance must be **removed or zeroed**, or it is an allowance that cannot be spent; (b) `ai_writing` must actually be **enforced** — nothing checks it today, so until it is, free users keep using the assistant exactly as before and the decision is decorative. ⚠️ **This is a behaviour REGRESSION for existing free users**, who can use AI writing today; it was flagged before the decision and taken deliberately                                                                                                                                                                                                                                                | D4 (now has a floor)               |
| **D4** | **Decision:** do the seven unenforced premium feature codes get a backend row?                                                                                                              | M         | [48 §5.2](./48_PlatformParityRegister.md). The catalogue sells eight `PremiumFeature` codes and the backend asserts exactly **one** (`ai_budget`); `PolicyEngineService.isEntitled()` has zero callers, so a subscriber's plan is computed correctly and then ignored on every route but the AI meter's. Until answered, **no client may gate on the seven** — a client-only wall in front of a route the server serves. ~~Depends on **D3**~~ — **D3 is answered (2026-08-08) and D4 is UNBLOCKED but the owner has DEFERRED its scope: "will decide this later what enforcement we will do."** What D3 fixed is that D4 now has a **floor rather than a blocker**: `ai_writing` must be enforced for D3 to mean anything, so the open question is no longer _whether_ to enforce but _how far past `ai_writing`_ to go. Three scopes were put to the owner and none chosen yet — (a) only what D3 needs: `ai_writing` + zero free's `ai_budget`; (b) all seven; (c) the five AI codes only, leaving `advanced_analytics` + `publishing_pro`. **The "no client may gate on the seven" rule still stands** for the six beyond `ai_writing` | B2 shares the enforcement path     |
...
prefs (✅ **done 2026-08-10** — sweep [48 §6.10](./48_PlatformParityRegister.md)); **P-2** (@mentions)
stays its own item because it touches BOTH clients; onboarding stays blocked on a product shape for web.
...
| Clap (1..50 accumulating) ✅ **DONE 2026-08-10 (W7b)**                                      | ~~mobile → web~~ **web-first** | **Direction reversed** — mobile has no clap control ([48 §3.15](./48_PlatformParityRegister.md) **M7-3**). Accumulating, debounce-batched into one `POST` carrying `count`, capped client-side at `MAX_CLAPS_PER_USER_PER_PIECE`, removal is all-or-nothing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
