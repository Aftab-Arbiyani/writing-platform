...
3. **Client implementation** against the frozen contract, reusing the `@umberleaf/ui` tokens, the `qk.*`
...
- **Excerpt derived server-side** from the TipTap JSON using `@umberleaf/utils`' existing `extractPlainText`.
...
| **api-types guard: not applicable**          | Checked (48 §3.11). `@umberleaf/api-types` contains **no** piece shapes at all — the web client types pieces locally — so the new DTO falls outside the guard's register and pinning it would mean introducing pieces to the package. The guard passes untouched                                                                                                                                                                      |
...
actions. Pinned by the §3.11 `@umberleaf/api-types` guard, which covers this DTO. `SettingsResponseDto` /
...
**Not covered by the §3.11 api-types guard**: `@umberleaf/api-types` has no collaboration namespace at
...
| Wire type   | `AskBookStreamEvent` added to `@umberleaf/api-types`, exempted by name in the §3.11 guard                                        |
...
**unchanged and is not pinned** by the `@umberleaf/api-types` guard ([48 §3.11](./48_PlatformParityRegister.md)),
...
**Bounds moved to `@umberleaf/shared` once** (`RETRIEVAL_CONFIG_BOUNDS`, `SEARCH_ANALYTICS_DEFAULT_WINDOW_DAYS`),
