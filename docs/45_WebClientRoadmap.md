...
| **W9** | **AF4 story consumers web port** — Story Explorer + Ask My Book                                                                                                                             | S–M       | **New row, opened 2026-08-07.** Both are AF4 consumers of the AF3 graph (`retrieval/consumers/story-explorer.controller.ts`, `retrieval/consumers/ask-book.controller.ts`), not the held analysis lifecycle — see the [§4.8](#48-w6--what-is-actually-left-in-it-corrected-2026-08-05) correction. Mobile ships both, exercised references exist for both (Ask My Book reachable since `qalam-mobile 5d055a5`/`e60a151`). An ordinary port, deliberately kept separate from W6's held analysis-lifecycle scope | —                                  |
