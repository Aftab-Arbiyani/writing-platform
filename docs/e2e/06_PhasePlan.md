...
| Monetization: subscribe → entitlement granted  |     |     |     | ✅  | `features/monetization` (`af5`) — W4 ([45 §4](../45_WebClientRoadmap.md), [report](../50_WebMonetizationReadinessReport.md)); suite runs with `VITE_ENABLE_MONETIZATION=true`. **The entitlement leg is proven end to end; the payment leg asserts the server's honest refusal, because no inert port exists** (§6) |
...
- **Monetization subscribe→entitlement** (row `af5`) — ~~there is no monetization/subscribe/billing
  feature or route in the frontend~~ **the UI shipped in W4** ([45 §4](../45_WebClientRoadmap.md)):
  `monetization.spec.ts` drives all five surfaces against the real stack, and they are covered by the
  a11y and visual dimensions in both themes.

  **This row's other stated premise turned out to be wrong, and correcting it is why the row is green in
  two halves rather than one.** The claim above — that "the inert payment port" makes a checkout
  assertable under the third-party allowance ([00 §6]) — does not hold: **there is no inert payment
  port.** Every adapter is key-gated (`StripeAdapter.isConfigured()` and its Apple/Google siblings each
  test a secret for emptiness) and `PaymentProvider.Manual` is in the vocabulary with **no adapter at
  all**, so without real credentials the registry answers `PAYMENT_PROVIDER_NOT_CONFIGURED` for every
  provider including `manual`. Verified live with the platform flag raised. The port does not no-op; it
  declines ([48 §3.6 W4-4](../48_PlatformParityRegister.md)).

  So the row asserts both halves without faking either:

  - **Subscribe** is driven through the real button to the real endpoint, and the assertion is the honest
    refusal with nothing charged — in both server states a deployment can be in
    (`MONETIZATION_DISABLED` while the pre-seeded flag is down, and `PAYMENT_PROVIDER_NOT_CONFIGURED`
    with it raised). That is this stack's actual contract, and it is the same shape as
    `assistant.spec.ts` asserting "AI is turned off".
  - **Entitlement granted → the gate opens** is proven end to end through an **admin entitlement
    override**: the same Entitlement Service, the same snapshot the client gates on, the same
    decision-cache invalidation a subscription transition triggers — reached without a card. Both
    directions are asserted (deny closes the gate, revoke reopens it), so the whole path is covered:
    admin write → cache invalidation → snapshot read → rendered gate.

  **What is still not asserted is a completed payment**, and the reason is environmental rather than a
  client gap: it needs a configured provider (a Stripe test key) in the E2E stack. Stubbing
  `/monetization/subscription` is ruled out by the no-mocks invariant ([README §invariants]). That is a
  **stack item**, tracked here — and unlike the AI stub it is cheap, because Stripe publishes test keys.

Both rows have now re-entered the matrix as `✅` — `af2` with W2, `af5` with W4. What remains in each
is a **stack** gap (an AI provider stub; a Stripe test key), scoped above and tracked here, not a client gap.
