...
| Publishing: review → approve → publish         |     |     |     | ✅  | `features/collaboration` (`af6`) — W3c; two actors, because approving is coarse-gated on the platform `publishing.approve` an author does not hold (defect **W3c-1**, [48 §3.4](../48_PlatformParityRegister.md))  |
