...
| AI conversations: create → rename → export → delete |     |     |     | ✅  | `features/ai` (`af1`) — W8 ([45 §4](../45_WebClientRoadmap.md), [report](../52_WebAiSurfacesReadinessReport.md)); all six routes through the real UI. Export asserted as a **download** (the route returns JSON, so the file is the client's doing). Runs as a **throwaway user** — every UI-created conversation is untitled, so on the shared writer the rows are ambiguous. **Create is the load-bearing assertion:** mobile ships this screen and cannot populate it ([48 §3.12 W8-1](../48_PlatformParityRegister.md)) |
| AI conversations: no archive control (deliberate)   |     |     |     | ✅  | `features/ai` (`af1`) — W8; asserts an **absence**. `PATCH status:'archived'` persists and hides nothing, because the list query has no status predicate ([48 §3.12 W8-2](../48_PlatformParityRegister.md)) — so the control is omitted on purpose, and this is the assertion to change if the backend gains a filter                                                                                                                                                                                                       |
| Prompt library: built-in shelf + save + persist     |     |     |     | ✅  | `features/ai` (`af2`) — W8; **no wire at all** on either client, verified not assumed ([48 §3.12](../48_PlatformParityRegister.md)). `localStorage` is the only copy that exists, so surviving a reload is the whole contract and nothing on the backend could assert it                                                                                                                                                                                                                                                    |
| AI token usage: the AF1 ledger, distinct from AF5   |     |     |     | ✅  | `features/ai` (`af1`) — W8; `GET /ai/usage/me`. Asserts the caps invariant (a window either has a valued `progressbar` or says it has no cap) and the **two-way cross-link** with W4's `/settings/billing/usage`, which reads the different `GET /monetization/usage` ([52 §3.4](../52_WebAiSurfacesReadinessReport.md))                                                                                                                                                                                                    |
