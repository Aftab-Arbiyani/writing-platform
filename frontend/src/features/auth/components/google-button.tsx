import { QButton } from '@qalam/ui';
import type { ReactElement } from 'react';

/** Google's multicolour "G" as a self-contained SVG (no external asset — CSP-safe). */
function GoogleGlyph(): ReactElement {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden focusable="false">
      <path
        fill="#4285F4"
        d="M17.64 9.2c0-.64-.06-1.25-.16-1.84H9v3.48h4.84a4.14 4.14 0 0 1-1.8 2.72v2.26h2.92c1.7-1.57 2.68-3.88 2.68-6.62Z"
      />
      <path
        fill="#34A853"
        d="M9 18c2.43 0 4.47-.8 5.96-2.18l-2.92-2.26c-.8.54-1.84.86-3.04.86-2.34 0-4.32-1.58-5.02-3.7H.92v2.33A9 9 0 0 0 9 18Z"
      />
      <path
        fill="#FBBC05"
        d="M3.98 10.72a5.4 5.4 0 0 1 0-3.44V4.95H.92a9 9 0 0 0 0 8.1l3.06-2.33Z"
      />
      <path
        fill="#EA4335"
        d="M9 3.58c1.32 0 2.5.45 3.44 1.35l2.58-2.58C13.47.9 11.43 0 9 0A9 9 0 0 0 .92 4.95l3.06 2.33C4.68 5.16 6.66 3.58 9 3.58Z"
      />
    </svg>
  );
}

/**
 * "Continue with Google" — starts the OAuth redirect dance (docs/32 §3.3). The click handler
 * does a top-level navigation (never a fetch), so this is a plain button.
 */
export interface GoogleButtonProps {
  onClick: () => void;
  loading?: boolean;
  disabled?: boolean;
}

export function GoogleButton({ onClick, loading, disabled }: GoogleButtonProps): ReactElement {
  return (
    <QButton
      variant="secondary"
      size="lg"
      block
      onClick={onClick}
      loading={loading}
      disabled={disabled}
    >
      <span className="flex items-center justify-center gap-2">
        {loading ? null : <GoogleGlyph />}
        Continue with Google
      </span>
    </QButton>
  );
}
